...
按功能轮次添加以下文件，不为规划创建空模块或空测试。第 2 轮已添加 TaskForm、TaskCard、useTasks、taskRules 以及两个实际测试文件；第 3 轮添加 taskStorage.js 与 taskStorage.test.js；第 4 轮添加 TaskBoard，替换原列表展示；第 5 轮添加 useTheme；README.md 和 review.md 仍为后续规划：
...
第 1 轮只建立骨架；第 2 轮实现内存任务列表与 CRUD；第 3 轮添加 localStorage 持久化和错误保护；第 4 轮添加三列看板与桌面原生拖拽；第 5 轮添加深浅主题记忆与必要的界面收尾，不扩展其他功能。
