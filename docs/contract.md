...
第 5 轮主题接入：useTheme 在 App setup 中读取 course-task-board.theme.v1，合法的 light/dark 优先恢复，无选择或非法值默认 light。初始化仅读取、不写入；点击按钮同步更新 document.documentElement 的 dark 类并保存选择。主题读写异常捕获并给出简短提示，当前页面仍可切换。Tailwind 4 使用 class 驱动的自定义 dark 变体，根元素 color-scheme 同步原生表单控件外观。主题状态独立于任务数据与保存路径。
